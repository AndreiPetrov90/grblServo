  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    switch(letter) {
      ...
      case 'G':
        ...
        switch(int_value) {
          ...
          case 4: case 53:
            ...
            break;
          case 0: case 1: case 2: case 3: case 38: case 100:
          ...
          case 80:
			...
			if(int_value == 100)gc_block.modal.motion = MOTION_MODE_SERVO;
            break;
      ...
      default:
        ...
        switch(letter){
          case 'A': // set servo level High
				if(int_value >= SERVO_HIGH && int_value <=SERVO_LOW){
					//OCR2A = int_value;
					//return(STATUS_OK);
					word_bit = WORD_A; 
					gc_block.values.a = int_value;
				}else FAIL(STATUS_GCODE_MAX_VALUE_EXCEEDED);
				break;
          // case 'B': // set Low servo
          ...
          case 'L': word_bit = WORD_L; gc_block.values.l = int_value; 
		  break;
  ...
  */
  ...
  // was explicitly commanded in the g-code block.
    ...
    // the value must be positive. In inverse time mode, a positive value must be passed with each block.
      ...
      if (gc_block.values.f == 0.0 && gc_block.modal.motion != MOTION_MODE_SERVO) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [Feed rate undefined]
  ...
  if (axis_command) { bit_false(value_words,(bit(WORD_A)|bit(WORD_X)|bit(WORD_Y)|bit(WORD_Z))); } // Remove axis words.
  ...
  */
  ...
  if (gc_state.modal.motion != MOTION_MODE_NONE) {
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      ...
      } else if (gc_state.modal.motion == MOTION_MODE_SERVO) {
        OCR2A = gc_block.values.a;
      }else{
