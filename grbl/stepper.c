...
void stepper_init()
{
  ...
  // Configure Timer 2: for 50Hz signal to servo. Use pin LimitZ as PWM output
  LIMIT_DDR |= (1<<Z_LIMIT_BIT); // ZLIMIT as output
  TCCR2B = (1<<CS21) | (1<<CS22) | (1<<CS20); // perscaler 1024
  TCCR2A = 0; // PWM outputs disapled
  TIMSK2 = (1<<OCIE2A) | (1<<TOIE2); // enable interrupt	
  OCR2A = SERVO_LOW; // init value (servo 0 or 180)
}
